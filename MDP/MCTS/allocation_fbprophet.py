...
import numpy as np
import time
from functions.fn_read_fbprophet import *
...
def proportional(s, x1, x2):
    return math.floor(min(x1, s * x1 / (x1 + x2))), math.floor(
        min(x2, s * x2 / (x1 + x2))
    )
...
class AllocationGame:
    def __init__(self, demands, base_stock):
        self.prp = 10
    ...
    def get_valid_allocations(self):
        ...
        valid_allocations = []
        for a1 in range(
            1, min(self.inventory, self.demands[self.prp - self.days_remaining][0]) + 1
        ):  # Limit allocation for customer 1 based on demand
            for a2 in range(
                1,
                min(
                    self.inventory - a1, self.demands[self.prp - self.days_remaining][1]
                )
                + 1,
    ...
    def get_reward(self, allocations):
        no_days = len(
            allocations
        )  # during the simulation process, from a node onward to the last day
        ...
        profit = self.p * (tot_alloc_1 + tot_alloc_2) - (
            self.penalty_per_shortage_1 * max(self.fill_rate_target - fill_rate_1, 0)
            + self.penalty_per_shortage_2 * max(self.fill_rate_target - fill_rate_2, 0)
        )
...
class MCTSNode:
    ...
    def __init__(self, game_state, parent=None, allocation=[0, 0], state_indice=[0, 0]):
        ...
        self.distance = math.sqrt(
            (self.demand[0] - self.allocation[0]) ** 2
            + (self.demand[1] - self.allocation[1]) ** 2
        )
    ...
    def explotation_score(self):
        return (self.reward_sum / self.visits if self.visits != 0 else 0) + model_param[
            "c1"
        ] * (-self.distance)
    ...
    def select(self):
        if model_param["augmented_select"] == False:
            ...
            exploration = [
                math.sqrt(math.log(child.parent.visits) / child.visits)
                if child.visits != 0
                else model_param["Big_M"]
            ...
            select_score = [
                x + math.sqrt(model_param["c"]) * y
                for x, y, in zip(explotation, exploration)
            ]
    ...
    def augmenting_ucb(self):
        ...
        if sum(today_demand) <= self.game_state.inventory:
            try:
                ...
                random_allocation = random.choices(
                    valid_allocations, weights=probs_select, k=1
                )[0]
            except:
            ...
            opt_a1, opt_a2 = self.AllocationPolicy(
                today_demand,
                fill_rate_1,
                fill_rate_2,
                game_state_copy.fill_rate_target,
                game_state_copy.inventory,
            )
            # Euc distance between optimal allocation and valid allocations
            distances = [
                -math.sqrt(
                    (opt_a1 - valid_allocations[_][0]) ** 2
                    + (opt_a2 - valid_allocations[_][1]) ** 2
                )
                for _ in range(len(valid_allocations))
            ]
            softmax_probs = softmax(distances, axis=0)
            random_allocation = random.choices(
                valid_allocations, weights=softmax_probs, k=1
            )[0]
    ...
    def expand(self):
        ...
        for alc in self.game_state.get_valid_allocations():
            child = MCTSNode(
                ...
                parent=self,
                allocation=alc,
                state_indice=[self.state_indice[0] + 1, counter],
            )
    ...
    def AllocationPolicy(self, today_demand, fill_rate_1, fill_rate_2, target_fr, S):
        ...
        tetha1 = (fill_rate_1 - target_fr) / (
            abs(fill_rate_1 - target_fr) + abs(fill_rate_2 - target_fr)
        )
        tetha2 = (fill_rate_2 - target_fr) / (
            abs(fill_rate_1 - target_fr) + abs(fill_rate_2 - target_fr)
        )
        if tetha1 == 0:
            tetha1 += 1e-10
        ...
        for child in root.children:
            if child.visits > 0:
                ...
                if model_param["augmented_select"] == False:
                    ...
                    reward_rate = child.explotation_score
                elif child.game_state.inventory >= sum(
                    child.game_state.demands[tree_level_no]
                ):
                    reward_rate = child.explotation_score
                    if child.allocation == (2, 5) and (
                        root.demand == (2, 5) or root.demand == (2, 6)
                    ):
                        print("hahaha")
                elif child.game_state.inventory < sum(
                    child.game_state.demands[tree_level_no]
                ) and child.game_state.inventory != sum(child.allocation):
                    ...
                    opt_a1, opt_a2 = child.AllocationPolicy(
                        child.demand,
                        fill_rate_1,
                        fill_rate_2,
                        child.game_state.fill_rate_target,
                        child.game_state.inventory,
                    )
                    # Euc distance between optimal allocation and valid allocations
                    distance = math.sqrt(
                        (opt_a1 - child.allocation[0]) ** 2
                        + (opt_a2 - child.allocation[1]) ** 2
                    )
                    reward_rate = (child.reward_sum / child.visits) - model_param[
                        "c1"
                    ] * distance
...
def main_run(demands, base_stock):
    # Reset the Class (remove all instances)
    MCTSNode.reset()

    # game = AllocationGame()
    game = AllocationGame(demands, base_stock)
    time.sleep(1)
    ...
    FR1 = round(
        sum([allocations[x][0] for x in range(len(allocations))])
        / sum([game.demands[x][0] for x in range(len(game.demands))]),
        2,
    )
    FR2 = round(
        sum([allocations[x][1] for x in range(len(allocations))])
        / sum([game.demands[x][1] for x in range(len(game.demands))]),
        2,
    )
    ...
    fr1_prop = round(
        sum([y[0] for y in allocations_prop]) / sum([x[0] for x in game.demands]), 2
    )
    fr2_prop = round(
        sum([y[1] for y in allocations_prop]) / sum([x[1] for x in game.demands]), 2
    )
...
for _ in range(model_param["num_repeats"]):
    # print(_)
    demands, bstock = demands_bstock_fbprophet(actual_demands, base_stock, _)

    jsResult = main_run(demands, bstock)
