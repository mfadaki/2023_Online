...
class AllocationGame:
    ...
    def get_reward(self, allocations):
        ...
        profit = self.p*(tot_alloc_1+tot_alloc_2)-(self.penalty_per_shortage * max(tot_demands_1 - tot_alloc_1, 0) + self.penalty_per_shortage * max(tot_demands_2 - tot_alloc_2, 0))
...
def mcts(game_state, iterations):
    ...
    for n in MCTSNode._registry:
        print(f'state_indice={n.state_indice}')
        print(f'demand =  {n.demand}')
        print(f'allocation =  {n.allocation}')
        print(f'visits =  {n.visits}')
        print(f'reward_sum =  {n.reward_sum}')
        if n.visits != 0:
            print(f'reward_sum/visits =  {n.reward_sum/n.visits}')
