...
class DQNAgent():
    ...
    def q_values_feasibility_update(self, state, q_values):
        ...
        for action_ in feasible_actions:
            if action_[0] > state_org[1] or action_[1] > state_org[2]:
    ...
    def state_action_feasibility(self, state):
        ...
        for action_ in feasible_actions:
            if action_[0] <= state_org[1] and action_[1] <= state_org[2]:
...
action_real = feasible_actions[action_index]
print(f'state = {state}')
print(f'Action/Allocation = {action_real}')

state = [10, 5, 5]
state_index = feasible_states.index(state)
state_tensor = states_one_hot_tensor[state_index]
